...
double dir_x = 0.0; // 车前进方向，用于车轮可视化
double dir_y = 0.0;
double dir_z = 0.0;
void visWhole();
...
int mark_id = -1;
void visCar();
const double lidarToIMUDist = 1.87; // 这个是雷达和惯导的距离
...
void doINSMsg(const common_msgs::HUAT_ASENSING::ConstPtr &msgs)
{
    ...
    {
        Carstate.car_state.theta = 0; // 后续需要考虑弧度还是角度
        oldAzimuth = msgs->azimuth;
    ...
    {
        // if(msgs->azimuth - oldAzimuth<= -180){
        //      Carstate.car_state.theta =  - (msgs->azimuth - oldAzimuth )*PII/180;
        // }
        double diff = -(msgs->azimuth - oldAzimuth);
        Carstate.car_state.theta = -(msgs->azimuth - oldAzimuth) * PII / 180; // 后续需要考虑弧度还是角度
        ...
        // std::cout << "nowAngle" << msgs->azimuth << std::endl;
        // std::cout << "firstAngle" << -oldAzimuth << std::endl;
        // std::cout << "different" << (diff) << std::endl;
...
void calc_vehicle_direction(double roll, double pitch, double yaw, double &x, double &y, double &z)
{
    ...
    // 转向角
    motion_direction = atan2(my_ins.east_velocity, my_ins.north_velocity);

    x = cos(tfPitch) * cos(yaw);
    y = sin(tfRoll) * sin(tfPitch) * cos(yaw) + cos(tfRoll) * sin(yaw);
    z = -tfRoll * sin(yaw) + tfPitch * sin(yaw);
}
...
void visWhole()
{
    ...
    // 四元数规格化
    tf::Vector3 axis(0, 0, 1); // 旋转轴固定为 z 轴
    if (theFirstTime)
    {
        oldAngle = (-motion_direction);
        theFirstTime = !theFirstTime;
    }
    else
    {
        angle = (-motion_direction);
        // move = angle - oldAngle;
        if (angle - oldAngle > 0.2618)
        {
            move = 0.2618;
        }
        else if (angle - oldAngle < -0.2618)
        {
            move = -0.2618;
        }
    }
    tf::Quaternion q_new(sin(PII / 4) * cos(angle / 2 + 1 * move), sin(PII / 4) * sin(angle / 2 + 1 * move), cos(PII / 4) * sin(angle / 2 + 1 * move), cos(PII / 4) * cos(angle / 2 + 1 * move));
    q_new.normalize();
    // q_new = q_old;
    // q_new = q_old * q_new ;
