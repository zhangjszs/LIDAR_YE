...
void lidar_cluster::vis_for_marker(const pcl::PointXYZ max, const pcl::PointXYZ min, float euc){
	// bbox_marker.clear();
    ...
    // text = "(" + to_string(centroid[0]) + "," + to_string(centroid[1]) + "," + to_string(centroid[2]) + ")";
    text = to_string(euc);

    euc_marker.text = text;
    euc_marker.scale.z = 0.2;
